 ...
 */
open class TargetPattern {
    
    fileprivate(set) open var targets: [Target]       // the pattern's targets
    fileprivate(set) open var gap: CGFloat            // gap between targets in degrees
    fileprivate(set) open var targetSize: CGFloat     // size of the targets in degrees
    fileprivate(set) open var count: Int              // count of targets in this pattern
    
    fileprivate let MaxDegrees: CGFloat = 360
